            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Frontend menu</h4>
                ...
                <div class="col-md-4">
                    <div class="card border-light mt-3 shadow">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Select Menu</h4>
                        </div>
                        ...
                        <form action="{{ route('back.menus.index') }}" method="GET">
                                    ...
                                    <select name="menu" class="form-select">
                                        ...
                                        @foreach ($menus as $menu)
                                            <option value="{{ $menu->id }}"
                                                {{ request('menu') == $menu->id ? 'selected' : '' }}>{{ $menu->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="card-footer">
                                <button class="btn btn-success">Select menu</button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="card border-light mt-3 shadow">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Add Menu Item</h4>
                        </div>
                            ...
                            <div class="accordion accordionFrontMenu" id="accordionFrontMenu">
                                <div class="card">
                                    <div class="card-header" id="headingOne">
                                        <h2 class="mb-0">
                                            <a class="btn btn-block text-left" data-bs-toggle="collapse"
                                                href="#collapseFMPages" role="button" aria-expanded="true"
                                                aria-controls="collapseFMPages">
                                                <b>Pages</b>
                                            </a>
                                        </h2>
                                    </div>

                                    <div id="collapseFMPages" class="collapse show" aria-labelledby="headingFMPages"
                                        data-parent="#accordionFrontMenu">
                                        <form action="{{ route('back.menus.storeMenuItem') }}" method="POST">
                                                    ...
                                                    @if (count($pages))
                                                        @foreach ($pages as $page)
                                                            <div class="custom-control custom-checkbox mr-sm-2">
                                                                <input name="relation_id[]" value="{{ $page->id }}"
                                                                    type="checkbox" class="custom-control-input"
                                                                    id="page_id_{{ $page->id }}">
                                                                <label class="custom-control-label"
                                                                    for="page_id_{{ $page->id }}">{{ $page->title }}</label>
                                                            </div>
                                                        @endforeach
                                                    @else
                                                        <p>No page created</p>
                                            ...
                                            <div class="card-footer">
                                                <button class="btn btn-success">Add to Menu <i
                                ...
                                <div class="card">
                                    <div class="card-header" id="headingFMCustom">
                                        <h2 class="mb-0">
                                            <a class="btn btn-block text-left" data-bs-toggle="collapse"
                                                href="#collapseFMCustom" role="button" aria-expanded="true"
                                                aria-controls="collapseFMCustom">
                                                <b>Custom Link</b>
                                            </a>
                                        </h2>
                                    </div>

                                    <div id="collapseFMCustom" class="collapse" aria-labelledby="headingFMCustom"
                                        data-parent="#accordionFrontMenu">
                                        <form action="{{ route('back.menus.storeMenuItem') }}" method="POST">
                                            @csrf
                        ...
                        <form action="{{ route('back.menus.menuItemPosition') }}" method="POST">
                                        ...
                                        <ol class="dd-list">
                                            @foreach ($menu_items as $menu_item)
                                                <li class="dd-item" data-id="{{ $menu_item->id }}">
                                                    <div class="dd-handle">{{ $menu_item->menu_info['text'] ?? '' }}
                                                    </div>

                                                    @if (!$menu_item->relation_with && $menu_item->url)
                                                        <span class="text-success edit_item_btn"
                                                            data-id="{{ $menu_item->id }}"><i
                                                                class="ri-edit-2-line"></i></span>
                                                    @endif

                                                    <a href="{{ route('back.menus.destroyItem', $menu_item->id) }}"
                                                        onclick="return confirm('Are you sure to remove?');"><i
                                                            class="ri-delete-bin-5-line float-right"></i></a>
                                                                        ...
                                                                        <ol class="dd-list">
                                                                            @foreach ($menu_item->Items as $menu_item)
                                                                                <li class="dd-item"
                                                                                    ...
                                                                                    @if (!$menu_item->relation_with && $menu_item->url)
                                                                                        <span
                                                                                            class="text-success edit_item_btn"
                                                                                            data-id="{{ $menu_item->id }}"><i
                                                                                                class="ri-edit-2-line"></i></span>
                                                                                    @endif
                            ...
                            @if (request('menu'))
                                <div class="card-footer">
                                    <div class="row">
                                        <div class="col-6">
                                            <a href="{{ route('back.menus.destroy', $menu->id) }}"
                    ...
                    <div class="modal-content">
                        ...
                        <form action="{{ route('back.menus.updateItem') }}" method="POST">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label>URL</label>
                                    <input type="url" name="url" class="form-control form-control-sm url_input"
                                        required>
                                </div>
                                <div class="form-group">
                                    <label>Link Text</label>
                                    <input type="text" name="text"
                                        class="form-control form-control-sm link_text_input" required>
                                </div>
                            </div>
