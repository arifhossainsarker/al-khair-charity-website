...
@php
    $page_title = 'Research';
    $researches = \App\Models\Research::where(['status' => 1])
...
@section('head')
    @include('meta::manager', [
        'title' => 'Research' . ' - ' . ($settings_g['title'] ?? env('APP_NAME')),
    ])
@endsection
                ...
                <div class="md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-4">
                    ...
                    @foreach ($researches as $research)
                        <div class="border border-gray-200 p-2 rounded-md mb-5 md:mb-0">
                            <div>
                                <a href="{{ route('single-research', $research->slug) }}">
                            ...
                            <div class=" py-2">
                                {{-- <span
                                    class="primary-color text-xs font-semibold py-4">{{ \Carbon\Carbon::parse($research->created_at)->format('d-M-Y') }}</span> --}}
                                <h2 class="pt-2 primary-color text-base font-semibold pb-2">
                                    <a href="{{ route('single-research', $research->slug) }}">{{ $research->title }}</a>
                                </h2>
                                ...
                                <a class="primary-color text-xs font-bold hover:underline"
                                    href="{{ route('single-research', $research->slug) }}">Read More</a>
                            </div>
                        </div>
                    @endforeach
                </div>
