...
class AuthController extends Controller
{
    ...
    public function emailVerifyCheck($id)
    {
        ...
        if($pre_user->email_verified_at){
            return redirect()->route('homepage')->with('error-alert', 'Your email has already verified');
        }
        ...
        // DB::table('pre_users')->where('email', $pre_user->mobile_number)->delete();

        Auth::login($user);

        return redirect()->route('memberDashboard')->with('success-alert', 'Your verification success!');
    }
